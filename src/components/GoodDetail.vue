<template>
	<div id="goodDetail">
		<headerBox :flag="'back'" :title="'商品列表'" :kefu="true"></headerBox>
		<section class="main-body">
			...
			<div class="main-info">
				...
				<!--收藏标志-->
				<div @click="loveCheck" class="love">
			...
			<!--详情、评论tab-->
			<div ref="tab" class="good-tab">
				<span @click="tabChange('1')" :class="{'tab-item':true,'active-tab':tab_flag=='1'}">商品详情</span>
				<span @click="tabChange('2')" :class="{'tab-item':true,'active-tab':tab_flag=='2'}">用户评论</span>
			</div>

			<!--商品详情-->
			<transition :duration="{enter:1000,leave:0}" mode="out-in" enter-active-class="animated fadeIn">
				<div v-show="tab_flag=='1'" class="good-detail">
					<div class="detail-text">
						<p class="detail-item"><span>品牌名称：</span><span>{{detail.brand_name}}</span></p>
						<p class="detail-item" v-for="item in display"><span>{{item.attr_name}}：</span><span>{{item.attr_value}}</span></p>
					</div>
					<div class="detail-img">
						<div class="detail-title">商品详情<span>︱ Details</span></div>
						<div id="concent" v-html="detail.content"></div>
					</div>
				</div>
			</transition>
			...
			<!--商品评论-->
			<transition :duration="{enter:1000,leave:0}" mode="out-in" enter-active-class="animated fadeIn">
				<div class="scroll" ref="scroll" @scroll="scroll" v-show="tab_flag=='2'">
					<comment :commentList.sync="comment" @comment="tab_flag='1'"></comment>
				</div>
			</transition>
		</section>
		...
		<transition mode="out-in" name="fade">
			<div v-show="buy_flag" class="modal"></div>
		</transition>
		<!--底部-->
		<footer class="footer">
			...
			<transition mode="out-in" name="slide">
				<div v-show="buy_flag" class="buy">
					...
					<div class="buy-header">
						...
						<!--关闭弹出-->
						<i @click="buy_flag=false" class="iconfont icon-cuo"></i>
					</div>
					...
					<div class="buy-select">
						<div class="select-title count">
							...
							<div class="count-number">
								<i @click="countNumber('-')" class="iconfont icon-minus"></i>
								<input @keyup="num" @change="num" type="text" v-model="count" />
								<i @click="countNumber('+')" class="iconfont icon-jia"></i>
							</div>
						</div>
					</div>
			...
			<div class="footer-btn">
				<span @click="buy('addToCar')" class="btn">加入购物车</span>
				<span @click="buy('buyNow')" class="btn">立即购买</span>
			</div>
		</footer>

		<!--back-to-top-->
		<a @click="backToTop" class="back-to-top">
...
<script>
	...
	export default {
			...
			this.$http({
				...
				.then(res => {
					...
					if(res.data.code == '200') {
						...
						this.love = res.data.store != null ? true : false
					} else {
		...
		watch: {
			tab_flag: function (newValue, oldValue) {
				if(newValue == '2') {
					this.$http({
						...
						.then(res => {
							if(res.data.code == '200') {
								...
								if(res.data.goods.length > 0) {
									this.comment = [...this.comment, ...res.data.goods]
