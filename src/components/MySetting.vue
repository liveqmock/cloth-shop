<template>
	<div class="my-setting">
		<headerBox :flag="'back'" :title="'设置'"></headerBox>
		<!--设置头像-->
		<div @click="upload" class="list-item">
			<span>我的头像</span>
			<div>
				<img :src="img" />
				<form id="form">
					<input id="img" name="avatar" v-show="false" type="file" accept="image/*;capture=camera" @change="preview">
				</form>
		...
		<!--设置昵称-->
		<div @click="show_nickname=true" class="list-item">
			<span>我的昵称</span>
			<div>
		...
		<!--绑定手机-->
		<div @click="user.phone==''?show_phone=true:''" class="list-item">
			<span>绑定手机</span>
			<div>
		...
		<!--变性-->
		<div @click="show_sex=true" class="list-item">
			<span>性别</span>
			<div>
		...
		<!--消息提示-->
		<popup @sure="changeNickName" :show.sync="show_nickname" :title="'修改昵称'" :message_show="false">
			<input class="nickname" type="text" v-model="nickname" placeholder="请输入要修改的昵称" />
		</popup>

		<popup @sure="changeSex" :show.sync="show_sex" :title="'修改性别'" :message_show="false">
			<p class="btn-group">
				<span @click="sex='1'" :class="{'btn-active':sex=='1'}">男</span>
				<span @click="sex='2'" :class="{'btn-active':sex=='2'}">女</span>
			</p>
		</popup>

		<popup @sure="changePhone" :show.sync="show_phone" :title="'绑定手机'" :message_show="false">
			<div class="bind">
				<div class="username">
					<div>+86</div>
					<input type="text" v-model="phone" placeholder="请输入你的电话号码" />
				</div>
				<div class="username">
					<div v-show="code_flag" @click="get_code">验证码</div>
					<div v-show="!code_flag">已发送({{code_time}})</div>
					<input type="text" v-model="code" placeholder="请输入你的验证码" />
				</div>
			</div>
		</popup>
