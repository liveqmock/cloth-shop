...
<div class="order">
	...
	<div v-if="order!=''"
	     class="brand">
		<div @click="$router.push({name:'sort',query:{key:'brand',value:order.brand,title:order.name}})"
		     class="brand-title">
			<div>
...
<div class="functionBar">
	...
	<div @click="payNow"
	     v-if="order.button_status=='1'"
	     class="btn settlement">
		立即付款
	</div>
	<div v-else-if="order.button_status=='4'"
	     @click="seeLogistics"
	     class="btn settlement">
		查看物流
	</div>
...
export default {
	...
	async created() {
		...
		if (res.data.code == '200') {
			...
			this.count = res.data.goods.reduce((a, b) => ~~a.goods_num + ~~b.goods_num)
		} else {
	...
	methods: {
		...
		async payNow(item) {
			...
			let number = this.goods.reduce((a, b) => a + b)
