<template>
	<div class="shopCar">
		<headerBox :flag="'search'" :title="'购物车'"></headerBox>
		...
		<!--没有商品-->
		<div v-show="goods.length==0" class="noGood">
			<i class="iconfont icon-yifu1"></i>
			<span>购物车没有商品，赶紧去挑选吧</span>
			<span @click="$router.replace({name:'home'})" class="btn">挑选商品</span>
		</div>
		<tab></tab>
	</div>
</template>
...
<script>
	...
	export default {
				...
				.then(res => {
					...
					if(res.data.code == '200') {
						if(res.data.goods.length > 0) {
							for(let g of res.data.goods) {
								this.$set(g, 'check', false)
							}
		...
		methods: {
			...
			check(i, index) {
				let arr = this.goods
				arr[i].goods[index].check = !arr[i].goods[index].check
				this.goods = []
				this.goods = arr
			},
			//选择品牌
