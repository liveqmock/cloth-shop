...
<div class="myLove">
	...
	<div class="love-tab">
		<span @click="tabChange('0')"
		      :class="{'tab-item':true,'tab-active':tab=='0'}">全部&nbsp;&nbsp;({{total_count}})</span>
		<span @click="tabChange('1')"
		      :class="{'tab-item':true,'tab-active':tab=='1'}">折扣&nbsp;&nbsp;({{promotion_count}})</span>
	</div>
...
export default {
	...
	methods: {
		...
		async scroll() {
			...
			if (res.data.code == '200' && res.data.goods.length) {
				console.log(res.data)
