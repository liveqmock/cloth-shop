...
<div class="withdraw">
	<headerBox :flag="'back'"
	           :title="'佣金提现'"></headerBox>
	<div class="withdraw-header">
		<span>我要提现</span>
		<div class="withdraw-amount"><span>￥</span><input v-model="money"
			       type="text"
			       @change="num"
			       @keyup="num" /></div>
	...
	<span @click="withdraw"
	      class="btn withdraw-btn">申请提现</span>
</div>
</template>
<script>
...
export default {
	...
	methods: {
		...
		async withdraw() {
			let res = await this.$http.get('/agency/withdraw');
			this.$store.dispatch({
				type: 'toast',
				message: res.data.msg,
				icon: res.data.code == 200 ? '1' : '2'
			})
		}
	}
}
